...
vehicleController.addImgDoc = async (req: Request, res: Response) => {
  const reponseJson: ResponseModel = new ResponseModel();
  const token: any = req.header("Authorization");
  ...
  const vehicl = await vehicles.findOne({ _id: id_vehicle });

  if (vehicl) {
    if (vehicl.imgs_documentation.length > 0) {
      document = vehicl.imgs_documentation;
      document.push(img_document);
      const vehicleUpdated = await vehicles.findByIdAndUpdate(id_vehicle, { imgs_documentation: document });
...
vehicleController.deleteImgDoc = async (req: Request, res: Response) => {
  const reponseJson: ResponseModel = new ResponseModel();
  ...
  const delImg = await vehicles.findByIdAndUpdate(id_vehicle, { $pull: { imgs_documentation: { public_id: public_id } } });
...
vehicleController.updateImgDoc = async (req: Request, res: Response) => {
  const reponseJson: ResponseModel = new ResponseModel();
  ...
  const filename = await uploadImageVehicle(image);
  const delImag = await deleteImageVehicle(public_id);
  const vehi =  await vehicles.findOne({ _id: id_vehicle });
  const delImg = await vehicles.findOneAndUpdate({ _id: id_vehicle, "imgs_documentation.public_id": public_id }, { $set: { "imgs_documentation.$.img": filename.secure_url, "imgs_documentation.$.public_id": filename.public_id } });
