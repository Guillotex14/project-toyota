...
adminRouter.get("/allSellers", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let arraySellers = [];
    let infoSellers = [];
    const ress = yield Users_schema_1.default.find({ type_user: "seller" }).sort({ date_created: -1 }).then((res) => __awaiter(void 0, void 0, void 0, function* () {
        if (res) {
            jsonRes.code = 200;
            jsonRes.message = "Vendedores encontrados";
            jsonRes.status = true;
            for (let i = 0; i < res.length; i++) {
                yield Sellers_schema_1.default.find({ id_user: res[i]._id }).then((res2) => {
                    if (res2) {
                ...
                for (let k = 0; k < infoSellers.length; k++) {
                    if (res[j]._id.toString() == infoSellers[k].id_user.toString()) {
                        let seller = {
                            ...
                            image: (yield imgUser_schema_1.default.findOne({ id_user: res[j]._id })) ? yield imgUser_schema_1.default.findOne({ id_user: res[j]._id }) : "",
                        };
...
adminRouter.post("/addSeller", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const exist = yield Users_schema_1.default.findOne({ email: reqAdd.email });
    if (exist) {
        reponseJson.code = 400;
        ...
        const newUser = new Users_schema_1.default({ email: reqAdd.email, password: hash, username: reqAdd.username, type_user: "seller" });
        const newSeller = new Sellers_schema_1.default({ fullName: reqAdd.fullName, city: reqAdd.city, concesionary: reqAdd.concesionary, date_created: date_created, phone: reqAdd.phone });
        yield newUser.save();
        if (newUser) {
...
adminRouter.post("/sellerById", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const jsonRes = new Response_1.ResponseModel();
    const { id } = req.body;
    const seller = yield Sellers_schema_1.default.findOne({ id_user: id });
    const userSeller = yield Users_schema_1.default.findOne({ _id: seller.id_user });
    if (seller) {
        let data = {
            _id: seller._id,
...
adminRouter.post("/deleteSeller", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const jsonRes = new Response_1.ResponseModel();
    const { id } = req.body;
    const ress = yield Users_schema_1.default.findOneAndDelete({ _id: id }).then((res) => __awaiter(void 0, void 0, void 0, function* () {
        if (res) {
            jsonRes.code = 200;
            jsonRes.message = "usuario eliminado exitosamente";
            jsonRes.status = true;
            yield Sellers_schema_1.default.findOneAndDelete({ id_user: res._id }).then((res2) => {
                if (res2) {
                    return res2;
                }
                else if (!res2) {
...
adminRouter.post("/addBrand", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const jsonRes = new Response_1.ResponseModel();
    const { name } = req.body;
    const exist = yield brands_schema_1.default.findOne({ name: name });
    if (exist) {
        jsonRes.code = 400;
        ...
        const newBrand = new brands_schema_1.default({ name: name });
        yield newBrand.save();
        if (newBrand) {
            jsonRes.code = 200;
