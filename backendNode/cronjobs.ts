...
const cronJobs = async () => {
    ...
    for (let i = 0; i < info.length; i++) {
        ...
        if (dateNow > dateSell) {
            await Vehicles.findOneAndUpdate({_id:info[i]._id},{sold:false, id_seller_buyer:null, date_sell:null, price_ofert:null, final_price_sold:null, name_new_owner: null, email_new_owner: null, phone_new_owner: null, dni_new_owner: null,dispatched: false });
        }
        
    }
}

const cronJobs2 = async () => {

    const info = await Vehicles.find({id_seller_buyer:{$ne:null},sold:false,price_ofert:{$ne:null}});
